...
class ImageOCR:
    def __init__(self, img):
        self.img = img
        # output list of dictionaries with recognized text in the format {'lang':'text'}
        self.text = []
        self._boxes = []

    def decodeImage(self) -> np.ndarray:
        '''Decode base64 image to numpy format'''
        self.img = cv2.imdecode(np.fromstring(
            self.img, np.uint8), cv2.IMREAD_UNCHANGED)
    ...
    def _resizeImage(self, image: np.ndarray, max_resolution: int) -> np.ndarray:
        ...
        new_dim = (int(image.shape[1] / scale_coef),
                   int(image.shape[0] / scale_coef))
    ...
    def _measureStrings(self) -> tuple:
        ...
        for slice in slices:
            ...
            th = 2
            H, W = crop.shape[:2]
            lines = [y for y in range(H - 2) if hist[y]
                     <= th and hist[y + 1] > th]
            if len(lines) > 0:
                font_size.extend([lines[i+1] - lines[i]
                                 for i in range(len(lines) - 1)])
                num_lines.append(len(lines))
        # Calculate the average line height in pixels
        mean_font_size = int(np.array(font_size).mean()
                             ) if len(font_size) > 1 else 0
        # Calculate the average number of text lines
        mean_num_lines = int(np.array(num_lines).mean()
                             ) if len(num_lines) > 1 else 0
    ...
    def _getTextMask(self, image: np.ndarray, font_size: int, num_lines: int) -> np.ndarray:
        ...
        if bool(font_size) == True:
            ...
            rect_kernel_size = (int(font_size*0.6),
                                int(font_size*0.4/h_text_coef))
            sq_kernel_size = (int(font_size*0.5),
                              int(font_size*0.4/h_text_coef))
    ...
    def _getBinaryImages(self, image: np.ndarray, font_size, new_font_size: int) -> list:
        ...
        self._boxes = [[0, 0, image.shape[1], image.shape[0]]] if len(
            self._boxes) <= 0 else self._boxes
        # Cut out blocks with text from the original image using bounding boxes
        for box in self._boxes:
            (x, y, w, h) = box
            cropped_img = image[y:y + h, x:x + w]
            # count the number of lines in the image
            num_lines = int(cropped_img.shape[0]/font_size)
            ...
            if num_lines > line_num_threshold:
                ...
                gaussian_blured = cv2.GaussianBlur(
                    cropped_img, (blur_size, blur_size), 10)
                # sharpen the image
                test_sharpened = cv2.addWeighted(
                    cropped_img, 1.5, gaussian_blured, -0.9, 0)
                ...
                medianblur = cv2.medianBlur(
                    test_sharpened, median_size)  # default 3
                # image binarization
                test_thresh = cv2.threshold(
                    medianblur, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)[1]
                # We calculate the percentage of black pixels relative to the total number of pixels
                percent_zeros = int(cv2.countNonZero(
                    test_thresh)*100/(test_thresh.shape[1]*test_thresh.shape[0]))
                # If the background of the picture is more than n% black, and the text is white, then invert the colors
                img_binary = cv2.bitwise_not(
                    test_thresh) if percent_zeros < 40 else test_thresh
                ...
                scaled_binary_img = self._resizeImage(
                    img_binary, new_max_resolution)
    ...
    def _findBoxes(self, mask_img: np.ndarray, quantity: int) -> None:
        ...
        self._boxes = self._findLargestBoxes(quantity=quantity) if len(
            self._boxes) > quantity else self._boxes
    ...
    def _imageCropper(self, image: np.ndarray, crop_coef: float) -> np.ndarray:
        ...
        cropped_img = image[int(new_y):int(new_y+new_h),
    ...
    def getText(self, text_lang, crop: bool, set_font: int) -> list:
        ...
        if crop == True:
            ...
            binary_images = self._getBinaryImages(
                self.img, font_size, set_font)
        else:
            binary_images = self._getBinaryImages(
                self.img, font_size, set_font)
        ...
        for image in binary_images:
            if text_lang == False:
                ...
                multilang_recog_text = self._recognizeText(
                    default_config, sample_image)
                # Detect language
    ...
    def drawBoxes(self, max_resolution) -> np.ndarray:
        ...
        for box in self._boxes:
            (x, y, w, h) = box
            cv2.rectangle(img_with_boxes, (x, y),
                          ((x + w), (y + h)), (255, 255, 0), 10)
